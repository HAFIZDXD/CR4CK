		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='HAFIZD-XD CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='HAFIZD-XD OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]HAFIZD XD OK[/bold green]'))
					ok+=1
					break
